    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden mb-3">
      ...
      >
        <div className="text-left">
          <div className="font-medium capitalize">{formatDateFr(dateKey)}</div>
          <div className="text-xs text-gray-500">{hasRepas ? `${dayData.repas.length} repas` : 'Aucun repas'}</div>
        </div>
        <div className="flex items-center gap-3">
          <span className={`font-bold ${dayData.total_jour > 0 ? 'text-green-700' : 'text-gray-400'}`}>
            {dayData.total_jour > 0 ? `${dayData.total_jour.toLocaleString('fr-FR')} kcal` : '—'}
          </span>
          {hasRepas && (
            <svg
            ...
            <div key={repas.id} className="px-4 py-2">
              <div className="flex justify-between items-start mb-1">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{repas.heure} · {repas.aliments.length} aliment{repas.aliments.length > 1 ? 's' : ''}</p>
                </div>
